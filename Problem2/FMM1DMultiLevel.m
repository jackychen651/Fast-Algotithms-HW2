function u = FMM1DMultiLevel(x, q, K)
...
for l = L:-1:1
    if l == L
        for j = 1:2^l
            q0_interval(j,l) = sum(q(I{j,l}).*cos(K*(x(I{j,l}))), 1);
            q1_interval(j,l) = sum(q(I{j,l}).*sin(K*(x(I{j,l}))), 1);
        end
    else
        for j = 1:2^l
...
for l=2:L
    for i = 1:2^l
        for j=1:2^l
            ...
            x_c = -1+(2*j-1)/2^l;
            hat_q_0 = q0_interval(j,l)*cos(K*x_c)+q1_interval(j,l)*sin(K*x_c);
            hat_q_1 = q1_interval(j,l)*cos(K*x_c)-q0_interval(j,l)*sin(K*x_c);
            for k=1:size(I{i,l},1)
                u(I{i,l}(k)) = u(I{i,l}(k)) + hat_q_0 * exp(1j*K*abs(x(I{i,l}(k))-x_c)) - 1j* hat_q_1 * sign(x(I{i,l}(k)) - x_c) *exp(1j*K*abs(x(I{i,l}(k))-x_c));
            end
        end
    end
end
